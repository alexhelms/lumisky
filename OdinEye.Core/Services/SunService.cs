...
public class SunService
{
    ...
    public (double Altitude, double Azimuth) GetPosition(DateTime date, double latitude, double longitude)
    {
        ...
        var lonWest = -1 * longitude * OdinEyeMath.Deg2Rad;
        var lat = latitude * OdinEyeMath.Deg2Rad;
        var d = Calendar.DaysSinceJ2000(date);

        var (ra, dec) = Sun.Coordinates(d);
        var hourAngle = Position.SiderealTime(d, lonWest) - ra;
        var az = Position.Azimuth(hourAngle, lat, dec) * OdinEyeMath.Rad2Deg;
        var alt = Position.Altitude(hourAngle, lat, dec) * OdinEyeMath.Rad2Deg;
    ...
    public SunTime? GetRiseSetTime(DateTime date, double sunAngle, double latitude, double longitude)
    {
        ...
        var sunAngleToHorizon = sunAngle * OdinEyeMath.Deg2Rad;
    ...
    {
        private const double EarthObliquity = 23.4393 * OdinEyeMath.Deg2Rad;
        ...
        public static double SiderealTime(double daysSinceJ2000, double lonWest) =>
            OdinEyeMath.Deg2Rad * (280.1470 + 360.9856235 * daysSinceJ2000) - lonWest;
    }
    ...
    private static class Sun
    {
        ...
        public static double MeanAnomaly(double daysSinceJ2000) =>
            (357.5291 + 0.98560028 * daysSinceJ2000) * OdinEyeMath.Deg2Rad;

        public static double EclipticLongitude(double meanAnomaly)
        {
            var equationOfCenter = (C1 * Math.Sin(meanAnomaly) + C2 * Math.Sin(2*meanAnomaly) + C3 * Math.Sin(3*meanAnomaly)) * OdinEyeMath.Deg2Rad;
            var earthPerihelion = EarthEclipticLongitude * OdinEyeMath.Deg2Rad;

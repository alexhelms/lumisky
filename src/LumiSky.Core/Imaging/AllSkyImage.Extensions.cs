...
public static class AllSkyImageExtensions
{
    ...
    ///
    /// For multi-channel images, data must be copied to and from LumiSky to OpenCV
    ...
    public static Mat To8bitMat(this AllSkyImage image)
    {
        ...
        else
        {
            ...
            // OpenCV multi-channel Mats will never point to the LumiSky memory.
            //
            // LumiSky: RRR | GGG | BBB
            //  OpenCV: BGR | BGR | BGR
    ...
    ///
    /// For multichannel images, data must be copied to and from LumiSky to OpenCV
    ...
    public static unsafe void FromMat(this AllSkyImage image, Mat mat)
    {
        ...
        if (image.Channels == 1)
        {
            nint lumiskyData = (nint) Unsafe.AsPointer(ref MemoryMarshal.GetReference(image.Data.GetSpan()));
            nint opencvData = mat.DataPointer;

            if (lumiskyData == opencvData)
            {
                // Nothing to do, opencv is modifying the LumiSky image directly.
                return;
            }
            else
            {
